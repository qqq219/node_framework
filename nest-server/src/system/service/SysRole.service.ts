...
import { ListToTree } from "src/common/utils/normal.tool";
...
export class SysRoleService {
  ...
  async create(createRoleDto: SysRoleDto) {
    ...
    const values = createRoleDto.menuIds === undefined?[]:createRoleDto.menuIds.map((id) => {
      return {
        roleId: res.roleId,
        menuId: id,
      };
  ...
  async update(updateRoleDto: SysRoleDto) {
    ...
    //TODO 后续改造为事务
    const entity = this.sysRoleMenuEntityRepository.createQueryBuilder('entity');
    const values = updateRoleDto.menuIds === undefined?[]:updateRoleDto.menuIds.map((id) => {
      return {
        roleId: updateRoleDto.roleId,
        menuId: id,
      };
  ...
  async dataScope(updateRoleDto: SysRoleDto) {
    ...
    const values = updateRoleDto.deptIds === undefined?[]:updateRoleDto.deptIds.map((id) => {
      return {
        roleId: updateRoleDto.roleId,
        deptId: id,
      };
