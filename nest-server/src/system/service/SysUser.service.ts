...
import { DataScopeEnum } from "src/common/model/enum/DataScopeEnum";
...
import { SYS_USER_TYPE } from "src/common/model/enum/SysEnum";
...
export class SysUserService {
  ...
  async getUserByUsername(username:string):Promise<SysUserEntity|null>{
  ...
  async allocatedList(query: AllocatedReq) {
    if(query.roleId === undefined){
      return  ResultData.fail(500, "数据缺失");
    }
  ...
  async create(createUserDto: SysUserDto) {
    ...
    const postValues = createUserDto.postIds === undefined?[]:createUserDto.postIds.map((id) => {
      return {
        userId: res.userId,
        postId: id,
      };
    ...
    const roleValues = createUserDto.roleIds === undefined?[]:createUserDto.roleIds.map((id) => {
      return {
        userId: res.userId,
        roleId: id,
      };
  ...
  async update(updateUserDto: SysUserDto, userId: number) {
    ...
    //过滤掉设置超级管理员角色
    updateUserDto.roleIds = updateUserDto.roleIds === undefined?[]:updateUserDto.roleIds.filter((v) => v !== 1);
    ...
    if (updateUserDto?.postIds?.length != undefined && updateUserDto?.postIds?.length > 0) {
      //用户已有岗位,先删除所有关联岗位
      ...
      const postEntity = this.sysUserPostEntityRepository.createQueryBuilder('postEntity');
      const postValues = updateUserDto.postIds === undefined?[]:updateUserDto.postIds.map((id) => {
        return {
          userId: updateUserDto.userId,
          postId: id,
        };
  ...
  async authRole(userId: number) {
    ...
    if(user === null){
      return ResultData.failDefault("未查询到用户");
    }
    const dept = await this.sysDeptEntityRepository.findOne({
      where: {
        delFlag: '0',
  ...
  async export(res: Response, body: SysUserReq, user) {
    ...
    Reflect.deleteProperty(body, "pageSize")
