...
import { FormatDate, GetNowDate } from "src/common/utils/normal.tool";
import { ConfigService } from "@nestjs/config";
import { arraysContains, capitalize, getColumnLength, lowercaseFirstLetter, StringUtils, uppercaseFirstLetter } from "src/common/utils/gen.tool";
import { templateIndex } from "src/common/template";
import { GenConstants } from "src/common/model/constant/GenConstants";
...
export class GenService{
  ...
  async findOne(id: number) {
    ...
    if(data == null){
      return ResultData.failDefault("数据不存在");
    }
  ...
  async genUpdate(genTableUpdate: GenTableDto) {
    if(!genTableUpdate || !genTableUpdate.columns){
      return ResultData.failDefault("参数不能为空");
    }
  ...
  async importTable(tables: string, req: any) {
    ...
    for (const table of tableList) {
      ...
      let flag = false;
      if(this.config.get("gen.autoRemovePre")){
        const prefixs:string[] | undefined =  this.config.get("gen.tablePrefix")
        try {
      ...
      const tableData: CreateGenTableDto = {
        ...
        className: className,
        packageName:  this.config.get("gen.packageName") as unknown as string,
        moduleName:  this.config.get("gen.moduleName") as unknown as string,
        ...
        functionAuthor: this.config.get("gen.author")  as unknown as string,
